...
class DeviceListPageState extends State<DeviceListPage>{
            ...
            builder: (BuildContext context,AsyncSnapshot snapshot){
              if(snapshot.hasData){
               ...
               return FloatingSearchBar(
                   ...
                   onQueryChanged: (query) {//搜索内容改变时，展示推荐结果
                     items = [];
                     if(query!=null&&query!=''){
                       for(int i=0;i<tmp.length;i++){
  ...
  List<FilterModel> adminFilters = [FilterModel('解锁',false),FilterModel('锁定',false)];
  List<FilterModel> operateFilters = [FilterModel('可操作',false),FilterModel('不可操作',false)];
  List<FilterModel> protocolFilters = [FilterModel('mqtt',false),FilterModel('modbus-tcp',false),FilterModel('modbus-rtu',false),FilterModel('HTTP',false),FilterModel('other',false)];
  //临时存放当前选中的所有类型的筛选标签
  ...
  void _onConfirmFilter(){
    ...
    for(int i = 0;i<tmp.length;i++){
      ...
      var operatingState = tmp[i]['operatingState']=='ENABLED'?'可操作':'不可操作';
